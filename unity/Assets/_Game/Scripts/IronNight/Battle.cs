    ...
    public partial class Battle : MonoBehaviour
    {
        ...
        void TickShells(float dt)
        {
            for (int i = shells.Count - 1; i >= 0; i--)
            {
                ...
                if (hit == null && convoy && !s.friendly && !s.bounced) foreach (var tr in trucks) { if (tr.dead) continue; var d = tr.transform.position - s.pos; d.y = 0f; if (d.sqrMagnitude < tr.spec.radius * tr.spec.radius) { hit = tr; break; } }
        ...
        void Damage(Vehicle v, float dmg, Vector3 at)
        {
            ...
            if (convoy && trucks.Contains(v)) { TruckLost(v); return; }
